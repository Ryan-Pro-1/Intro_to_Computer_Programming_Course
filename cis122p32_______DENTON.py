...
def house():
    ''' Draws house based on input specs'''

    house_color = 'chocolate'
    x = 0
    y = 0
    jump(x, y)
    poly(4, 99, house_color)

    roof_color = 'red'
    y += 99
    jump(x, y)
    poly(3, 99, roof_color)
    
    door_color = 'brown'
    x += 40
    y = y - 99
    jump(x, y)
    poly(4, 30, door_color)
    
    return

def house_main():
    '''Main driver for code to draw a house'''
...
'''THE FOLLOWING IS THE ART CHALLENGE PART OF THE ASSIGNMENT.
NOTE THAT THE CODE IS LONG SINCE WE DONT HAVE FUNCTION FOR RECTANGLES'''
