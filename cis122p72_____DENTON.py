...
def rats(weight, p):
    ...
    weeks = 0
    rate = p * .01
    double_weight = 2 * weight    #created to variable
    
    while weight < (double_weight): #added colon & updated variable in parentheses
        weight += weight * rate
        weeks += 1              #changed 'wks = weeks + 1' to 'weeks += 1'
        
    weight = round(weight, 1)
    print(f'The rat weighs {weight} after {weeks} weeks.')
                                #added a space on this line
    return
